...
| 19 | `zombiePlugin.py` | SCHEDULED | Ignores cancellation, force-killed by watcher | `progress_callback` never checks DB status. `getData()` sleeps for `HEARTBEAT_TIMEOUT * 2`. User sets status to DISABLED mid-execution. The child process ignores the cancellation, so the watcher's next per-tick DB status check (§5.2) force-terminates it (`proc.terminate()`, then `proc.kill()` as fallback). Because the status change was user-initiated, DISABLED is preserved — NOT overwritten to ERROR — and the force-kill is recorded as an EventLog entry with exit_code=2. Tests the force-termination path under a user-initiated DISABLED. |
