...
def _dkb_test_pass3_heal_flush_batch(db, sub, ds):
    """Pass III must flush a batching sink (like LightRagSink) so a buffered
    heal upsert actually lands on the remote and in the DB row."""
    from unittest.mock import patch
    from worker.sink_recon import _pass3_heal_remote
    sub_row = db.get_subscription(sub)
    sub_name = _get_sub_name(db, sub)
    path = _write_output_file_test(sub_name, "healb.md", "# heal batch\n")
    ds_row = db.get_target(ds.id)
    ds_row.api_key = db.decrypt_target_api_key(ds_row)
    ...
    if svc._pending:
        return False, "heal upsert left buffered after Pass III"
    if not any(c[0] == "update_datafile" for c in svc.calls):
        return False, "update_datafile not invoked for heal"
    t_df2 = db.get_target_datafile(ds.id, df.id)
    if t_df2 is None or t_df2.remote_datafile_id != _MOCK_REMOTE_UPDATED:
