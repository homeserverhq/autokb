"""DKB reconciliation engine — orchestrates file-to-remote sync for a subscription.

Called by the worker from two paths:
  1. At the end of a FULL subscription run (after debounce + re-eval).
  2. Directly from a DKB_ONLY queue operation.
"""
...
from utils.dkb_service_base import compute_file_hash
...
def _send_error_email(sub_name: str, datastore_name: str, error: str) -> None:
    from worker.execution_engine import _send_smtp_for_worker
    try:
        _send_smtp_for_worker(
            subject=f"[AutoKB] DKB datastore error: {datastore_name}",
            body=(
...
def reconcile_subscription_datastores(
    sub, db: DatabaseManager, dkb_registry: DKBRegistry,
    queue=None, log=None,
) -> None:
    """Run DKB reconciliation for *all* datastores linked to this subscription.
    ...
    """
    if log is None:
        log = get_logger("dkb_recon", LOG_FILE)
    sub_id = sub.id
    sub_name = sub.name

    # Gather datastore_subscriptions for this sub
    ds_links = db.list_datastores_for_subscription(sub_id)
    if not ds_links:
        return  # no downstream datastores
    ...
    for ds_link in ds_links:
        ...
        if ds_status == STATE_DELETED:
            # Remove all datastore_datafile rows for this deleted link
            ds_df_rows = db.list_datafiles_for_datastore(datastore_id)
            for ds_df in ds_df_rows:
                remove_count += 1
                _invoke_remove(
                    ds_link, datastore_id, ds_df, db, dkb_registry, log, sub_name, error_ds_names
                )
                prog += 1
                _hb(db, sub_id, queue, pct=prog % 100)
            db.delete_datastore_subscription(datastore_id, sub_id)
            # After removal, check if this was the last subscription for the datastore
            remaining = db.count_datastore_subscriptions_for_datastore(datastore_id)
            if remaining == 0:
                _remove_orphan_datastore(datastore_id, db, dkb_registry, log)
            continue

        # --- ENABLED / ENQUEUED: full reconcile ---
        try:
            # 1. Ensure remote_datastore_id exists
            ...
            if svc is None:
                continue
            if not svc.remote_datastore_id:
                try:
                    svc.base_add_datastore()
                except Exception as exc:
                    _transition_ds_error(
                        ds_link, datastore_id, sub_id, db, log,
                        sub_name, svc.name, str(exc), error_ds_names,
                    )
                    continue

            pass1_count = _reconcile_pass1(
                fs_files, ds_link, datastore_id, sub_id, db, svc, queue, log,
            )
            ...
            if ds_status == STATE_ENQUEUED:
                db.set_datastore_subscription_status(datastore_id, sub_id, STATE_ENABLED)

            msg = f"Reconciled: +{pass1_count[0]} added, ~{pass1_count[1]} updated, -{pass1_count[2]} removed"
            db.set_datastore_subscription_status(
                datastore_id, sub_id, STATE_ENABLED, message=msg,
            )
            ...
            _transition_ds_error(
                ds_link, datastore_id, sub_id, db, log,
                sub_name,
                (ds_row.name if ds_row else datastore_id),
...
def _get_service(ds_row, db, dkb_registry, log):
    """Get service name and instantiate the DKB service for a datastore."""
    svc_row = db.get_dkb_service(ds_row.service_id)
    if svc_row is None:
        log.warning("dkb_service_not_found", datastore_id=ds_row.id, service_id=ds_row.service_id)
        return None
    api_key = db.decrypt_datastore_api_key(ds_row)
    ...
    svc = dkb_registry.load_service_for_recon(svc_row.name, patched, db)
    if svc is None:
        log.warning("dkb_class_not_found", datastore_id=ds_row.id, service=svc_row.name)
    return svc
...
def _reconcile_pass1(
    ...
    """Pass I.1 (FS→datastore) and I.2 (datastore→FS removal)."""
    add_count = 0
    update_count = 0
    remove_count = 0
    ds_df_rows = db.list_datafiles_for_datastore(datastore_id)
    ...
    for fpath, st in fs_files.items():
        ...
        if df is None:
            ...
            try:
                ...
                log.warning("dkb_add_failed", path=fpath, error=str(exc))
            continue
        ...
        if _file_matches_db(fpath, df):
            if ds_df and ds_df.hash != df.hash:
                try:
                    ...
                    log.warning("dkb_update_failed", datafile_id=df.id, error=str(exc))
            continue

        try:
            real_hash = compute_file_hash(fpath)
        except Exception as exc:
            log.warning("dkb_hash_failed", path=fpath, error=str(exc))
            continue
        ...
        if real_hash == df.hash:
            ...
            try:
                ...
                log.warning("dkb_update_failed", datafile_id=df.id, hash=real_hash, error=str(exc))
    ...
    for ds_df in ds_df_rows:
        if ds_df.datafile_id not in processed_datafile_ids:
            try:
                ...
                log.warning("dkb_remove_failed", datafile_id=ds_df.datafile_id, error=str(exc))
...
def _pass2_akb_datafiles(sub, db, output_dir, fs_files, log) -> None:
    ...
    for df in datafiles:
        if df.path not in fs_files:
            # File missing → try delete (swallow FK errors from disabled datastores)
            try:
        ...
        if _file_matches_db(df.path, df):
            ...
            try:
                ...
                log.warning("dkb_pass2_hash_failed", path=df.path, error=str(exc))
                db.update_datafile_last_checked(df.id)
                continue
            db.update_datafile_stats(df.id, st.st_size, st.st_mtime, real_hash)


def _invoke_remove(ds_link, datastore_id, ds_df, db, dkb_registry, log, sub_name, error_ds_names):
    """Remove a single datastore_datafile row (caller owns exception handling)."""
    ds_row = db.get_datastore(datastore_id)
    if ds_row is None:
        return
    svc = _get_service(ds_row, db, dkb_registry, log)
    ...
    try:
        ...
        _transition_ds_error(
            ds_link, datastore_id, ds_link.subscription_id, db, log,
            sub_name, ds_row.name, str(exc), error_ds_names,
        )


def _transition_ds_error(ds_link, datastore_id, sub_id, db, log,
                          sub_name, ds_name, error, error_ds_names) -> None:
    """Set a datastore_subscription to ERROR and send one email on transition."""
    if f"{datastore_id}_{sub_id}" in error_ds_names:
        return  # already handled this pass
    if ds_link and ds_link.status != STATE_ERROR:
        try:
            db.set_datastore_subscription_status(datastore_id, sub_id, STATE_ERROR, message=error)
        except Exception:
            pass
        try:
            ...
            pass
        error_ds_names.append(f"{datastore_id}_{sub_id}")

    log.warning("dkb_ds_error", datastore_id=datastore_id, sub_id=sub_id, error=error)


def _remove_orphan_datastore(datastore_id: str, db: DatabaseManager,
                              dkb_registry: DKBRegistry, log) -> None:
    """Called when the last subscription is removed from a datastore."""
    ds_row = db.get_datastore(datastore_id)
    if ds_row is None:
        return
    svc = _get_service(ds_row, db, dkb_registry, log)
    if svc and svc.remote_datastore_id:
        try:
            svc.remove_datastore()
        except Exception as exc:
            log.warning("dkb_remove_datastore_failed", datastore_id=datastore_id, error=str(exc))
